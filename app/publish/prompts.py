...
### note.comでの見やすさルール
- 各セクション間に必ず<hr>を入れて視覚的に区切る
- 決算ハイライトは1行1指標で見やすく（各行を<p><strong>売上高</strong>：100億円（前年比+15%）</p>の形式）
- SWOT分析は4つのh3見出し（強み/弱み/機会/脅威）で分け、各項目をul/liで箇条書き
- 重要な数字や結論は必ず<strong>タグで強調
- 長い段落は避け、1段落3-4文以内に抑える
- 記事末尾に読者への呼びかけを1文入れる：「この分析が参考になったらスキをお願いします。フォローで最新分析をお届けします。」
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "hashtags": {
                ...
                "description": (
                    "ハッシュタグ（#なし、8個以内）。"
                    "必須タグ: 決算分析, 日本株, 投資 の3つは必ず含める。"
                    "残り5つで企業名, 証券コード, 業種, トピック等を追加。"
                ),
            },
        },
...
DISCLAIMER = (
    ...
    "<p>この分析が参考になったら<strong>スキ</strong>をお願いします。"
    "<strong>フォロー</strong>で最新のAI決算分析をお届けします。</p>"
